...
export async function getMyHousing(req, res) {
  try {
    ...
    // Obtener vivienda del beneficiario (por id_usuario_beneficiario)
    const { data: viviendasData, error: errorVivienda } = await supabase
      .from('viviendas')
      .select(`
        id_vivienda,
        numero_vivienda,
      ...
      `)
      .eq('id_usuario_beneficiario', beneficiarioUid)
      .order('id_vivienda', { ascending: true })
      .limit(1)
      
    if (errorVivienda) {
      console.error('Error de base de datos al consultar vivienda:', errorVivienda)
      throw errorVivienda
    }
    const vivienda = Array.isArray(viviendasData) && viviendasData.length ? viviendasData[0] : null
    ...
    // Obtener información del proyecto por separado si es necesario
    let proyecto = null
    if (vivienda.id_proyecto) {
...
export async function getMyIncidences(req, res) {
  try {
    ...
    const incidencias = await getIncidencesByBeneficiary(beneficiarioUid)
...
export async function createNewIncidence(req, res) {
  try {
    ...
    const incidenciaData = {
      ...
      id_usuario_beneficiario: beneficiarioUid,
      categoria,
      descripcion,
      prioridad,
      estado: 'abierta',
    ...
    const { data: incidencia, error: errorIncidencia } = await supabase
      ...
      .select(`
        *,
        ...
        usuarios!incidencias_id_usuario_tecnico_fkey(nombre, email)
      `)
