...
export async function createNewProject(req, res) {
  try {
    const { 
      nombre, 
      ubicacion, 
      descripcion, // Este campo no existe en la BD, lo ignoramos por ahora
      fecha_inicio, 
      fecha_fin, // Mapear a fecha_entrega
      fecha_entrega,
      estado, // Este campo no existe en la BD, lo ignoramos por ahora
      coordinador_uid, // Este campo no existe en la BD, lo ignoramos por ahora
    ...
    const projectData = {
      ...
      // Usar fecha_fin si está presente, sino fecha_entrega
      fecha_entrega: fecha_fin || fecha_entrega || null,
