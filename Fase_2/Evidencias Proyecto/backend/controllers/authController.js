...
import { findUserByEmail, findUserByRut, getLastUser, insertUser, getUserById, updateUser } from '../models/User.js'
import { storeRecoveryCode, validateRecoveryCode, markRecoveryCodeAsUsed } from '../models/PasswordRecovery.js'
import { sendRecoveryEmail } from '../services/EmailService.js'
import { isStrongPassword, isValidRutFormat, normalizeRut } from '../utils/validation.js'
  ...
  try {
    ...
    if (!usuario) {
      if (process.env.DEBUG_AUTH === '1') {
        console.log(`[LOGIN][DEBUG] Email no encontrado: ${email.toLowerCase()}`)
      }
    ...
    const passwordMatch = stored && stored.startsWith('$2') 
...
export async function resetPassword(req, res) {
  try {
    ...
    console.log(`[RESET] Contraseña actualizada exitosamente para: ${emailLower}`)
